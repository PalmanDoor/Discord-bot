        ...
        if message.content.startswith('//help'):
            ...
            if closest_folder:
                ...
                if len(text_files) > 0:
                    ...
                    response = text
                    image_files = [file for file in file_list if file.endswith(('.png', '.jpg', '.jpeg', '.pdf', '.doc', '.docx', 'mp3'))]
                    for image_file in image_files:
                        file_path = os.path.join(folder_path, image_file)
                        file = discord.File(file_path)
                        file.spoiler = False  # Можно установить spoiler=True, если нужно, чтобы изображение было скрытым
                        message.attachments.append(file)  # Добавляем вложение к сообщению

                    await message.channel.send(response, files=message.attachments)
                else:
                    await message.channel.send('По вашему запросу найдены только изображения.')
            else:
            ...
            # Запрос на получение предметов пользователя
            query_items = f"SELECT item_trade FROM trade WHERE id_trade='{user_id}'"
            cursor.execute(query_items)
            items = cursor.fetchall()

            # Запрос на получение денег пользователя
            query_money = f"SELECT money_user FROM users WHERE id_user='{user_id}'"
            ...
            if items:
                ...
                items_text = '\n'.join(items_list)
                inventory_embed = discord.Embed(title='Ваш инвентарь', description=items_text, color=discord.Color.blue())
                inventory_embed.add_field(name='Деньги', value=str(money[0]))
                await message.channel.send(embed=inventory_embed)
            else:
                await message.channel.send('Ваш инвентарь пуст.')
